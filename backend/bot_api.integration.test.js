...
test('shared immutable votes feed only the authenticated user taste profile', { timeout: 60000 }, async (t) => {
  ...
  assert.equal(liked.reaction.dislikedByMe, false);
  const unliked = await api(`/api/posts/${reactionPost.id}/like`, null, { userId: a.user.id });
  assert.equal(unliked.reaction.likedByMe, false);
  const disliked = await api(`/api/posts/${reactionPost.id}/dislike`, null, { userId: a.user.id });
  assert.equal(disliked.reaction.dislikedByMe, true);
  assert.equal(disliked.reaction.likedByMe, false);
  const switched = await api(`/api/posts/${reactionPost.id}/like`, null, { userId: a.user.id });
  assert.equal(switched.reaction.likedByMe, true);
  assert.equal(switched.reaction.dislikedByMe, false);
